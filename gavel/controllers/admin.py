        ...
        try:
            def tx():
                db.session.execute(ignore_table.delete(
                    ignore_table.c.item_id == item_id))
...
def parse_upload_form():
    ...
    if f and allowed_file(f.filename):
        ...
        if extension == "xlsx" or extension == "xls":
            ...
            data = list(utils.cast_row(worksheet.row_values(rx, 0, 3))
                        for rx in range(worksheet.nrows) if worksheet.row_len(rx) == 3)
        elif extension == "csv":
...
def annotator_patch():
    ...
    if action == 'Add Category':
        ...
        if not category:
            return utils.user_error('Category %s not found ' % request.form['category_id'])
        annotator.categories.append(
            AnnotatorCategory(annotator.id, category.id))
...
def category_detail(category_id):
    ...
    if not category:
        ...
        decisions = Decision.query.filter(
            Decision.category_id == category.id).all()
        counts = {}
        item_counts = {}
        for d in decisions:
            a = d.annotator_id
...
def item_detail(item_id):
    ...
    if not item:
        ...
        categories = Category.query.order_by(
            Category.id).filter(~Category.id.in_(category_ids))

        acs = AnnotatorCategory.query.filter(
            AnnotatorCategory.next == item).all()
        assigned = [ac.annotator for ac in acs]
        item_categories = ItemCategory.query.filter(
            ItemCategory.item == item).all()
        viewed_ids = {}
        ...
        if viewed_ids:
            skipped = AnnotatorCategory.query.filter(
                AnnotatorCategory.ignore.contains(
                    item) & ~AnnotatorCategory.annotator_id.in_(viewed_ids)
            )
        else:
            skipped = AnnotatorCategory.query.filter(
                AnnotatorCategory.ignore.contains(item))
    ...
    if not annotator:
        ...
        if ignored_ids:
            skipped = Item.query.join(Item.categories).filter(
                Item.id.in_(ignored_ids) & ~ItemCategory.viewed.contains(
                    annotator)
            )
        else:
            skipped = []
